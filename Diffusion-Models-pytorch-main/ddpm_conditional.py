...
class Diffusion:
        ...
        for i, (images, classes, labels) in enumerate(pbar):
            with torch.autocast("cuda") and (torch.inference_mode() if not train else torch.enable_grad()):
                ...
                if np.random.random() < 0.15:
                    classes = None
                    labels = None
                predicted_noise = self.model(x_t, t, classes, labels)
    ...
    def save_model(self, run_name, epoch=-1):
        "Save model locally"
        torch.save(self.model.state_dict(), os.path.join("last_models", run_name, f"checkpt_class.pt"))
        torch.save(self.ema_model.state_dict(), os.path.join("last_models", run_name, f"ema_checkpt_class.pt"))
        torch.save(self.optimizer.state_dict(), os.path.join("last_models", run_name, f"optim_class.pt"))
